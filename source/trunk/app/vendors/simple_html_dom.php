...
class simple_html_dom_node {
	...
	function markAsRead($recursive = false){
		$this->isRead = 1;
    	if($recursive){
    		$allNodes = array();
    		if($this->children){
    			foreach($this->children as $child){
    				$child->markAsRead(true);
    			}
    		}
    	}
    }
    
    function markAsUnread($recursive = false){
    	$this->isRead = 0;
    ...
    protected function parse_selector($selector_string) {
        ...
        foreach ($matches as $m) {
             ...
             * @auther: ducva 
            */
        	if(!empty($m[3])) {$key='class'; $val=str_replace(".", " ", $m[3]);}
